...
public class AppSettings {
   ...
   static void U() {
      String var0 = JsonParser.AboutDialogCloseListener(cK, true);
      ...
      try {
         ...
         Logger.error("Could not save configuration file", var12);
      }
   ...
   public static void init(boolean var0) {
      ...
      Logger.k(new File(var1, "NMSSaveEditor.log"));
      Logger.debug("Java Vendor: " + System.getProperty("java.vendor"));
      Logger.debug("Java Version: " + System.getProperty("java.version"));
      Logger.debug("Java Architecture: " + System.getProperty("os.arch"));
      Logger.debug("Operating System: " + System.getProperty("os.name"));
      Logger.debug("Working Dir: " + var1.getAbsolutePath());
      cK = new JsonObject();
      cL = false;
      if (cC.exists()) {
         try {
            ...
            if (var22.length > 0 && var22[0] == 123) {
               cK = JsonObject.FileComboBoxModel(new String(var22, "UTF-8"));
            } else {
                        ...
                        if (var7.equals("InventoryScaling")) {
                           ...
                           if (var7.endsWith(".Location")) {
                              ...
                              if ((var5 = var9.indexOf(44)) > 0) {
                                 ...
                                 cK.AccountPanel(var7 + ".X", var10);
                                 cK.AccountPanel(var7 + ".Y", var11);
                              }
                              ...
                              if ((var5 = var9.indexOf(44)) > 0) {
                                 var10 = Integer.parseInt(var9.substring(0, var5));
                                 var11 = Integer.parseInt(var9.substring(var5 + 1));
                                 cK.AccountPanel(var7 + ".Width", var10);
                                 cK.AccountPanel(var7 + ".Height", var11);
                              }
                           } else if (var7.equals("JSONEditor.Divider")) {
                              var10 = Integer.parseInt(var9);
                              cK.AccountPanel(var7, var10);
                           } else if (var7.startsWith("SteamID.")) {
                              var7 = var7.substring(8);
                              var6.f(Long.parseLong(var7));
                              cK.AccountPanel("KnownPlayers." + var7, var9);
                           } else {
                              cK.AccountPanel(var7, var9);
                           }
                        }
                     }
            ...
            Logger.AboutDialog("Could not load configuration file", var19);
         }
      }
   ...
   public static void V() {
      ...
      try {
         ...
         Logger.AboutDialog("Could not set look and feel: " + var1, var13);
         return;
      }

      Logger.debug("Look and Feel: " + UIManager.getLookAndFeel().getName());
