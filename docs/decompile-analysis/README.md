...
## Index
- [Storage and JSON model](storage-and-json-model.md) — `fq`, `ft`, `fs`, `eY`, `eV`, `fe`
- [Startup flow](startup.md) — `Application.main()` and `w` Runnable (to be added)

> Note: `storage-and-json-model.md` should be updated/replaced with the decompile-derived content for `fq/ft/fs/eY/eV/fe` (the current version appears to be placeholder/generic).
